...
ap_progra = data.loc[data['회사명'] == "카카오", "답변"]
...
ap_progra = data.loc[data['회사명'] == "네이버", "답변"]
nouns = okt.nouns(''.join(str(ap_progra.fillna(''))))
nouns = [n for n in nouns if len(n) > 1]
nouns = [n for n in nouns if not(match('^[0-9]',n))]
count = Counter(nouns)
...
ap_progra = data.loc[data['회사명'] == "농협은행", "답변"]
nouns = okt.nouns(''.join(str(ap_progra.fillna(''))))
nouns = [n for n in nouns if len(n) > 1]
nouns = [n for n in nouns if not(match('^[0-9]',n))]
count = Counter(nouns)
...
ap_progra = data.loc[data['회사명'] == "엔씨소프트", "답변"]
nouns = okt.nouns(''.join(str(ap_progra.fillna(''))))
nouns = [n for n in nouns if len(n) > 1]
nouns = [n for n in nouns if not(match('^[0-9]',n))]
count = Counter(nouns)
...
ap_progra = data.loc[data['회사명'] == "삼성에스디에스", "답변"]
nouns = okt.nouns(''.join(str(ap_progra.fillna(''))))
nouns = [n for n in nouns if len(n) > 1]
nouns = [n for n in nouns if not(match('^[0-9]',n))]
count = Counter(nouns)
